...
class RealData_PatchData(data.Dataset):
    def __init__(self, feat, k, k_global, cal_equiv_dist=False, use_endpoints_dist=False, 
        ...
        self.k = k # k_local
        self.k_global = k_global  # k_global
        self.cal_equiv_dist = cal_equiv_dist 
        self.use_endpoints_dist = use_endpoints_dist # TODO: Is this similar to farthest point sampling
        ...
        # self.rough_num_fiber_each_iter = 100  # this will be a rough targeted number of fibers in each iteration.
...
class unrelatedHCP_PatchData(data.Dataset):
    def __init__(self, root, out_path, logger, split='train', num_fiber_per_brain=10000,num_point_per_fiber=15, 
        ...
        # if True:
        #     num_streamlines = len(self.features)
        #     num_dims = self.features[0].shape[1]
        #     original_x = np.arange(self.features[0].shape[0])
        #     new_x = np.linspace(0, original_x[-1], num_point_per_fiber)
        #     interpolated_streamlines = np.zeros((num_streamlines, num_point_per_fiber, num_dims))
        #     stacked_streamlines = np.stack(self.features)

        #     for dim in range(num_dims):
        #         cs = [CubicSpline(original_x, stacked_streamlines[i, :, dim]) for i in range(num_streamlines)]
        #         interpolated_streamlines[:, :, dim] = np.array([cs[i](new_x) for i in range(num_streamlines)])
        #     self.features=interpolated_streamlines
        ...
        self.org_feat, self.org_label, self.local_feat, self.global_feat, self.new_subidx,self.new_feat_ras = self._cal_info_feat()
    ...
    def _cal_info_feat(self):
        ...
        return fiber_feat, fiber_label,local_feat, global_feat, new_subidx,fibre_feat_ras
