...
import os
import json
from utils.dVAE.tools import builder
from utils.dVAE import misc, dist_utils
import time
from utils.dVAE.logger import *
from utils.dVAE.AverageMeter import AverageMeter
from utils.dVAE.metrics import Metrics
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build dataset
    # train dataloader
    print('starting loading dataset----flag1')
    if config.total_bs!=1:
    ...
    print('loading dataset completed----flag2')
    # build model
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        if config.total_bs==1 and config.dataloader=='TractLoader':
            train_dataset.shuffle_custom_batch()
        if config.dataloader=='s3ClusterLoader': # when having custom batch (for tract data)
            train_dataset.shuffle_dict()
        ...
        batch_time = AverageMeter() # stores an average value a specific entity overtime, returns average value
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            # print(data.size())
            if config.total_bs==1: # because this is 4 dimensional [1, b, n, 3]
                data=torch.squeeze(data)
                # taxonomy_ids=torch.squeeze(taxonomy_ids)
                # model_ids=torch.squeeze(model_ids)
            ...
            points = data.cuda()
            # if dataset_name == 'ShapeNet' or dataset_name== 'TractLoader' or dataset_name=='s3ClusterLoader' or dataset_name=='unrelatedHCP_PatchData':
            #     points = data.cuda()
            # else:
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            else: # when using custom batch (for tract data)
                metrics = validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config,test_dataset, logger=logger)
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config,test_dataset=None, logger = None):
    ...
    if test_dataset is not None: # when using custom made batch(for tract data)
        if test_dataset=='csClusterLoader':
            test_dataset.shuffle_dict()
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if (idx+1) % 2000 == 0:
                ...
                # print_log('Test[%d/%d] Taxonomy = %s Sample = %s Losses = %s Metrics = %s' % 
                #           (idx + 1, n_samples, taxonomy_id, model_id, '%.4f' % test_losses.val(), '%.4f' % _metrics), logger=logger)
    ...
    if dataset_name=='s3ClusterLoader':
        shapenet_dict = json.load(open('./tract_data/shapenet_synset_dict.json', 'r'))
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            points = data.cuda()
            # if dataset_name == 'ShapeNet' or dataset_name=='s3ClusterLoader':
            #     points = data.cuda()
            # else:
            #     raise NotImplementedError(f'Train phase do not support {dataset_name}')
