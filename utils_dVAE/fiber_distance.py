...
# TODO: this local neighbours calculation can be replaced by the frechet distance or chamfer distance
...
def MDF_distance_calculation(point_set,ds_point_set=None,cal_equiv=False):
    ...
    #TODO: why is ds_point_set_equiv a flip of the ds_point_set? 
    ...
    if cal_equiv:
        ...
        new_flip_mask = torch.where(new_mean_dist_equiv < new_mean_dist, 1, 0)  
        # if equivalent distance is smaller than original distance, flip the fiber
...
def MDF_distance_calculation_endpoints(point_set, ds_point_set=None, cal_equiv=True):
    ...
    # Most efficient implementation
    time_start = time.time()
    # endpoints of each point_set and ds_point_set are used to calculate fiber_distance 
