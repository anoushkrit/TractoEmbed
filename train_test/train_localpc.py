...
def train_val_DL_net(net):
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for i, data in tqdm(enumerate(train_loader, start=0)):
            if data[0].shape[0] == args.train_batch_size:
...
def results_logging(args, logger, eval_state, label_names, org_labels_lst, org_predicted_lst,
    ...
    if not args.use_tracts_training:
        ...
        try:
            ...
            f = open("AccuracyOUT_PNet", 'a')
            f.write(f"Accuracy = {org_label_best_acc} f1 score = {org_label_best_mac_f1}\n")
            f.close()
        except:
    ...
    if args.use_tracts_testing:
        ...
        try:
            ...
            f = open("AccuracyOUT_PNet", 'a')
            f.write(f"Accuracy = {tract_label_best_acc} f1 score = {tract_label_best_mac_f1}\n")
            f.close()
        except:
            logger.info('Warning!! Number of classes, {}, does not match size of target_names, {}. Try specifying the labels parameter'
